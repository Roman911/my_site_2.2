...
export default class PropertiesComponent extends Component {
  render() {
    ...
    const arrValuePhoto = review.map(item => item.currentIndexPhoto);

    const arrValueRetouch = review.map(item => item.currentIndexRetouch);

    let resultPhoto = arrValuePhoto.reduce((sum, current) => sum + current, 0);

    let resultRetouch = arrValueRetouch.reduce((sum, current) => sum + current, 0);
    ...
    const props = properties.map((item, index) => <AboutProperties
          key={index}
